...
for matrix_size in matrix_sizes:
    ...
    mem_usage = memory_usage((solve_equation, (matrix_A, vector_B)), max_usage=True, retval=True)[0]
    mem_usage_increment = mem_usage - mem_usage_before

    results.append((matrix_size, elapsed_time, mem_usage_increment))

# Save results to CSV
np.savetxt("numpy_results.csv", results, delimiter=",", header="MatrixSize,Time,Memory", comments="", fmt=['%d', '%.6f', '%.6f'])
